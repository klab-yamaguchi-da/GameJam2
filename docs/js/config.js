...
const LEVELS = [
    ...
    },
    {
        ...
        platforms: [
            ...
            { x: 100, y: 480, width: 700 },    // 2層目
            { x: 0, y: 380, width: 650 },      // 3層目
            { x: 70, y: 280, width: 50 },      // 3段目の小さな足場（酔っ払い用）
            { x: 150, y: 280, width: 650 },    // 4層目
            { x: 0, y: 180, width: 550 },      // 5層目
            { x: 250, y: 80, width: 550 }      // 最上層
        ],
        ladders: [
            ...
            { x: 500, y: 180, height: 100 },
            { x: 300, y: 80, height: 100 },
            // 2段目に短いはしご3つ（x: 260, 390, 480）
            { x: 260, y: 480, height: 20 },  // 2段目
            { x: 390, y: 480, height: 20 },  // 2段目
            { x: 480, y: 480, height: 20 },  // 2段目
            // 3段目、4段目に長さ20のはしご
            { x: 300, y: 380, height: 20 },  // 3段目
            { x: 450, y: 380, height: 20 },  // 3段目（途中の短いはしご）
            { x: 370, y: 280, height: 20 }   // 4段目（30ピクセル左に移動: 400 -> 370）
        ],
        donkeyKongs: [
            { x: 300, y: 30 },   // 既存の酔っ払い（最上層）
            { x: 70, y: 230 }    // 新規の酔っ払い（3段目の小さな足場）
        ],
        princess: { x: 700, y: 50 },
        playerStart: { x: 730, y: 550 },
        trashBins: [
            { x: 10, y: 530 },   // ゴミ箱（最下層の左端）
            { x: 160, y: 230 }   // 新規のゴミ箱（4段目の左端）
        ],
        speedMultiplier: 1.0,
